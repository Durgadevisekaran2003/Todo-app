...
const ComingReminders = () => {
    ...
    // Sort reminders by closest date and time
    const sortedReminders = reminders
        .filter(reminder => reminder.email === userEmail) // Filter reminders for the current user
        .sort((a, b) => {
            const aDateTime = new Date(a.date.getFullYear(), a.date.getMonth(), a.date.getDate(), a.time.getHours(), a.time.getMinutes());
            const bDateTime = new Date(b.date.getFullYear(), b.date.getMonth(), b.date.getDate(), b.time.getHours(), b.time.getMinutes());
            return aDateTime - bDateTime; // Sort by the earliest time first
        });
    ...
    return (
        <div className="droppable" onDragOver={onDragOver} onDrop={drop}>
            <div className="u-reminder-header">
                ...
                <div className="u-reminder-text">TO BE DONE!</div>
            </div>
            {
                editIndex !== -1 &&
                <EditPopup />
